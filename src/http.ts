...
function startHttpServer(): void {
  ...
  const httpServer = createHttpServer(async (req, res) => {
    ...
    if (url.pathname === '/health') {
      ...
      // mode, credentials arrive per-request via X-CustomerThermometer-*
      // headers, not at startup - checking getCredentials() here would 503
      // the container permanently regardless of whether any customer has
      // connected yet.
    ...
    const apiKey = isGatewayMode ? (req.headers['x-customerthermometer-api-key'] as string | undefined) : undefined;
